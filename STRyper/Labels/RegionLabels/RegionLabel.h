...
///	A label that represents a ``Region``, either a molecular marker or a bin.
///
...
///
/// A  region label uses its ``ViewLabel/layer`` property and other core animation layers to display itself.
///
/// It becomes ``ViewLabel/hovered`` as appropriate, and is ``ViewLabel/highlighted`` only when the mouse button is released (on `mouseUp`).
///
...
@interface RegionLabel : ViewLabel <NSPopoverDelegate, NSControlTextEditingDelegate>
{
	/// A color used to represent the label given its state.
    NSColor *defaultColor, *disabledColor, *hoveredColor, *bandColor, *activeColor, *edgeColor;
...
typedef enum EditState : NSUInteger {
	...
	editStateShownSamples,
	
	/// Denotes that an action on the label will affect samples all samples from the same run as those shown by the view
	editStateRun,
	
	/// Denotes that an action on the label will affect all samples from the folder that is selected.
	editStateFolder
} EditState;
...
///
/// This is called by ``ViewLabel/doubleClickAction:``.
///
...
///
/// This can be used within `-drawRect` or `-updateLayer` to adapt to the application theme
- (void)updateForTheme;
